...
class Loading_GUI:
    ...
    def playlist_starter(self):
        ...
        self.Downloader_GUI.Playlist_results_GUI(lst = self.ytd.playlist_information)
...
class Downloader_GUI:
    ...
    def Download_video(self):
        if self.btCheckbar.i == 0:
            ...
            self.loading = self.Loading_GUI(self.tl,True,self.URL.get())
            self.Showinfo()
        else:
            self.background.destroy()
            self.tl = Toplevel(self.window)
            self.loading = self.Loading_GUI(self.tl,False,self.URL.get())
    ...
    def Playlist_results_GUI(self, lst=[]):
        ...
        for i in range(len(self.thumbnails)):
            ...
            b1.config(font = ("Helvetica", 12), fg = 'white', bg = 'black')
    ...
    def Loading_GUI(self, master, issingle, url):
        self.window2 = master
